...
Let's look at a high-level example of a simple audio program that decodes a list of files
...
Implementing the [`std::io::Read`](https://doc.rust-lang.org/std/io/trait.Read.html) interface allows us to fill a buffer with our
...
- a `Downloader` struct - will run in a new thread to download the content and store it in a temporary file
...
impl io::Seek for StreamReader {
    fn seek(&mut self, relative_position: SeekFrom) -> io::Result<u64> {
        ...
        self.reader.seek(relative_position)
    }
}
```
...
We'll use a data structure called a `RangeSet` for this - it stores disjoint sets of numeric ranges.
...
For example, if we download bytes `0` through `1000`, seek to `2000`, then download `1000` more
...
impl StreamProgress {
    fn new(seek_tx: mpsc::Sender<u64>) -> Self {
        Self {
            downloaded_ranges: Arc::new(RwLock::new(RangeSet::new())),
...
used to allow two threads to safely wait and signal for state changes.
...
impl StreamProgress {
    ...
    fn wait_for_position(&self) {
        ...
        // If we're not done downloading yet, reset the position_reached variable before we unlock the mutex.
...
impl Downloader {
    ...
    async fn download(&self) -> Result<()> {
        let stream = self.http_client.get(self.url).await.bytes_stream();
    ...
    fn seek_range(&self, seek_position: u64) -> Result<ByteStream> {
...
fn main() {
    ...
    while let Some(url) = next_input().await {
        ...
        let downloader = Downloader::new(url, temp_file, stream_progress, seek_rx);
...
impl RingBufState {
    ...
    fn offset_read_position(&self, val: usize) -> usize {
        (val + (self.read_position % self.size)) % self.size
    }

    // Same thing for the write position
    fn offset_write_position(&self, val: usize) -> usize {
...
supply enough data so it doesn't re-read old data during subsequent
iterations.
